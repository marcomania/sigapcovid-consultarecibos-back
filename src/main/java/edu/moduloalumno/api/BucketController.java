...
public class BucketController {
	 ...
	 * "file") MultipartFile file) throws IOException { return
	 * this.amazonClient.uploadFile(file); }
	 */

	@PostMapping("/uploadFile/{id}")
	public String uploadFile(@PathVariable String id, @RequestPart(value = "file") MultipartFile file)
			throws IOException {
	...
	public ResponseEntity<byte[]> getFile(@PathVariable String id) {
		/*
		 * byte[] data = s3Factory.getFile(file); ByteArrayResource resource = new
		...
		return ResponseEntity.ok().header("Content-disposition", "attachment; filename=\"" + keyname + "\"")
	...
	public ResponseEntity<ArrayList<FileAWS>> getFileFromFolder(@PathVariable String id) {
		...
		// amazonClient.getFileFromFolder(keyname);
		// ByteArrayResource resource = new ByteArrayResource(data);
		try {
